...
async function main() {
    ...
    for (let i = 0; i < users.length; i++) {
        ...
        if (!resi && sum.toString() !== '0') {
            console.log('UNMATCH null', user.userName, '0', sum.toString(), sum.toString())
            unmatches.push(user.userName)
            continue
        }

        if (!!resi && resi.amount !== sum.toString()) {
            console.log('UNMATCH nequ', user.userName, resi.amount, sum.toString(), sum.sub(resi.amount).toString())
