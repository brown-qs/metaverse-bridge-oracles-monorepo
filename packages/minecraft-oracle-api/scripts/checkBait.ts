...
async function main() {
    ...
    for (let i = 0; i < users.length; i++) {
        ...
        for (let j = 0; j < user.assets.length; j++) {
            ...
            if (asset.collectionFragment.collection.assetAddress === '0x1b30a3b5744e733d8d2f19f0812e3f79152a8777' && asset.assetId === '14') {
                sum = sum.add(asset.amount)
            }
            //console.log('    sum frag', sum.toString())
        }
        //console.log('    ', sum.toString())
        const resi = await connection.manager.getRepository(ResourceInventoryEntity).findOne({ where: { id: `${user.uuid}-1285-0x1b30a3b5744e733d8d2f19f0812e3f79152a8777-14` } })

        if (!resi && sum.toString() !== '0') {
            console.log('UNMATCH null', user.minecraftUserName, '0', sum.toString(), sum.toString())
            unmatches.push(user.minecraftUserName)
            continue
        }

        if (!!resi && resi.amount !== sum.toString()) {
            console.log('UNMATCH nequ', user.minecraftUserName, resi.amount, sum.toString(), sum.sub(resi.amount).toString())
            await connection.manager.getRepository(ResourceInventoryEntity).update(resi.id, { amount: sum.toString() })
            unmatches.push(user.minecraftUserName)
        }
    }
    console.log('Not matched', unmatches)
