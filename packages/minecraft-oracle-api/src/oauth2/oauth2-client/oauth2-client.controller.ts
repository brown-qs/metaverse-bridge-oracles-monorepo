import { Body, Controller, ForbiddenException, Get, Inject, Param, Patch, Post, UnprocessableEntityException, UseGuards } from '@nestjs/common';
...
export class Oauth2ClientController {
    ...
    async createClient(@User() user: UserEntity, @Body() body: Oauth2CreateClientDto): Promise<Oauth2ClientDto> {
        ...
        try {
            ...
            if (String(e).includes("duplicate key value violates")) {
                throw new UnprocessableEntityException("There is already a client with this name")
            } else {
                throw e
            }
        }
    ...
    async updateClient(@User() user: UserEntity, @Body() body: Oauth2ClientUpdateDto, @Param() params: { clientId: string }): Promise<Oauth2ClientDto> {
        ...
        if (invalidProps.length > 0) {
            throw new UnprocessableEntityException(`${invalidProps.join(", ")} can't be updated`)
        }

        if (!hasUpdatableProp) {
            throw new UnprocessableEntityException("You didn't provide anything that can be updated")
        }
