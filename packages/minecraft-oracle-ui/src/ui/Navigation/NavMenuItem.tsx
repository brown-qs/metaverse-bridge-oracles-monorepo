...
export default function NavMenuItem({ href, label, external = false }: { href: string, label: string, external?: boolean }) {
  return (
    <Link _hover={{ "textDecoration": "none" }} sx={{ textTransform: "uppercase", letterSpacing: "0.0032em", fontSize: "12px", color: "white", "&:hover": { color: "#F7CB49" } }} href={href} target={`${external ? '_blank' : '_self'}`}>
      <HStack>
