...
type CustomizerCardProps = {
    imageUrl: string,
    width: number,
    children?: ReactNode
}

const ComponentCustomizerCard = ({ imageUrl, width, children }: CustomizerCardProps) => {
  ...
  100%, 40% {
    ...
    return (
        <VStack spacing="0" w={width} minH={height} h={height} borderRadius="12px" overflow="hidden" boxShadow="md">
            <Box w={width} h={width} bg="blackAlpha.300" position="relative">
                <Box opacity={(fullImageLoaded || fullImageError) ? "0" : "1"} top="0" right="0" bottom="0" left="0" position="absolute" animation={`${skeletonAnimation} 1.6s linear infinite`}></Box>
            ...
            <VStack flex="1" w="100%" justifyContent="space-between" bg="gray.700" padding="8px" paddingTop="12px">
                {children}
            </VStack>
        </VStack >)
}

export default ComponentCustomizerCard
