...
const CustomizerCard = ({ imageUrl, height, width, children, selected }: CustomizerCardProps) => {
  ...
  100%, 40% {
    ...
    //_hover={{ border: selected ? "2px solid var(--chakra-colors-teal-200)" : "2px solid var(--chakra-colors-whiteAlpha-600)" }}
    return (
        <VStack spacing="0" w={width} minH={height} h={height} borderRadius="12px" overflow="hidden" border={selected ? "2px solid var(--chakra-colors-teal-200)" : "2px solid transparent"} >
